...
{
    struct pollfd fd = { ConnectionNumber(_glfw.x11.display), POLLIN , .revents = 0};
...
static GLFWbool waitForAnyEvent(double* timeout)
{
    ...
    struct pollfd fds[3] =
    {
        { ConnectionNumber(_glfw.x11.display), POLLIN, .revents = 0},
        { _glfw.x11.emptyEventPipe[0], POLLIN, .revents = 0}
    };

#if defined(__linux__)
    if (_glfw.joysticksInitialized)
        fds[count++] = (struct pollfd) { _glfw.linjs.inotify, POLLIN, .revents = 0};
#endif
